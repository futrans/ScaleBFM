...
from scaleretarget.utils.lafan_vendor.extract import read_bvh_multi_channel
...
from scaleretarget.utils.shape_optimizer import optimize_humoto_shape

class HumotoLoader(BaseLoader):
    ...
    def _load_sample(self, sample_path):
        ...
        bvh_data = read_bvh_multi_channel(sample_path)
        ...
        for frame in range(bvh_data.pos.shape[0]):
            ...
            result["LeftFootMod"] = (result["mixamorig:LeftFoot"][0], result["mixamorig:LeftToeBase"][1])
            result["RightFootMod"] = (result["mixamorig:RightFoot"][0], result["mixamorig:RightToeBase"][1])
    ...
    def _optimize_shape(self, bvh_file):
        ...
        if self.use_optimized_shape:
            ...
            self.scale = optimize_humoto_shape(
    ...
    def _get_root_height(self, bvh_data):
        ...
        foot_index = [bvh_joint_names.index(j) for j in ['mixamorig:LeftFoot', 'mixamorig:LeftToeBase', 'mixamorig:RightFoot', 'mixamorig:RightToeBase']]
