...
class Receiver:
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 9763, timeout: float = 3.0):
        ...
        self.ori = np.zeros((14, 4), dtype=np.float64) # (wxyz)
        self.pos = np.zeros((14, 3), dtype=np.float64) # weishuai: hardcoded for 14 selected links; you may change this to any number you have trained on
        self.time_code = 0
        self.frame_count = 0
    ...
    def parse_data_packet(self, message: bytes) -> bool:

        raise NotImplementedError

        # # TODO: This part is for you to parse data packet sent by any sources: Xsens, PICO ...
        ...
        #     logging.debug(f"Data packet parse error: {e}")
        #     return False
    ...
    def socket_receiver(self) -> None:
        ...
        while self.running:
            try:
                message = self.client_socket.recv(8 * 2000) # TODO: sometimes you may need to adjust this size
                if not message:
                    break
                self.parse_data_packet(message)
            except (socket.error, OSError, BrokenPipeError) as e:
                logging.info(f"Data socket stopped: {e}")
                self.running = False
                break
